...
│
├── config.py         197   Section 3  全定数 + キャラクター/パワーのレジストリ
├── engine.py         443   Section 1  唯一の Screen、スプライト、シーン切替、描画補助
├── entity.py         263   Section 2  両モード共通の Snake クラス + パワーの実行部
├── audio.py          281              ノンブロッキングな効果音（ファイルが無ければ無音）
│
├── screens.py        421              メニュー・キャラセレ・リザルト 2 種
├── solo.py           209              1 人プレイ        Section 2, 3, 4, 5
├── battle.py         476              2 人対戦          Section 2, 3, 4, 5
│
...
| 表示 | 位置 | 意味 |
|---|---|---|
| ハート + 名前 + スコア | 左上 / 右上 | HP とこのラウンドのスコア |
| 自分の色のゲージ | 名前の下 | パワーゲージ（最大 100） |
| **パワーのアイコン** | **ゲージの横** | 選んだパワー。`Q` / `O` で何が出るか |
| `P1  1 - 0  P2` | 中央・金色 | マッチのラウンドスコア |
| `STUN` / `BOOST` / `CLOAK` | **蛇の上に浮かぶ** | その蛇の現在の状態 |

> 以前 `STUN` はパワーゲージの横に出ていたため、*スキル*の状態のように読めて
> 分かりにくくなっていました。今は実際に目を向けている蛇の頭の上に浮かび、
> ゲージの横には選んだパワーのアイコンが入りました。蛇が上の壁に近いときは
> ラベルが自動で頭の**下**に回り、HUD の帯と重なりません。
...
**フルーツ** → スコア +`FRUIT_SCORE`（FEAST 中は ×2）、胴体 +1 節、パワーゲージ増加。
2P はフィールドに常に 2 個、互いに 40 px 以上離し、**どちらの蛇の胴体の上にも
出現しません**（以前は出現し得たため、取った側が自己衝突扱いでハートを失っていました）。
...
> **衝突に勝った側は「噛みつき」として罰されません。** 負けた側は頭がぶつかった
> まさにその位置でスタンするため、勝った側の頭はその首に触れたままになります。
> そこで勝った側に 30 フレームの噛みつき猶予を与えます。噛みつきのダメージだけを
> 抑えるもので、無敵ではありません。切れた後に相手の胴体を噛めば通常どおり HP が減ります。
>
...
### MVP =（スコア + 生存ボーナス）× 生き残った HP

各ラウンドの終了時に、ゲームはプレイヤーごとに 2 つを記録します。**そのラウンドで稼いだ
スコア**と、**終了時に残っていた HP** です。そして次のように計算します:

```
ラウンド MVP =（そのラウンドのスコア + SURVIVE_BONUS）× 残り HP
マッチ MVP   = 全ラウンドの合計
```

`SURVIVE_BONUS = 100` は、**残っているハート 1 つがそれ自体で 100 点の価値を持つ**
という意味です。以前は低スコアのままハートだけでラウンドを取っても MVP が 0 になり、
実際に起きたことを反映していませんでした。今は 3 HP 残して勝てば最低 300 になります。

変わらない点: **ノックアウトされたラウンド（HP = 0）は MVP が 0** です。どれだけ
スコアを稼いでいてもすべてが 0 倍されます。500 稼いでから倒されれば持ち帰るのは 0。
「生き残ること」は「稼ぐこと」と同じ重みを持ちます。

| ラウンド | スコア | 残り HP | MVP |
|---|---|---|---|
| R1 | 400 | 3 | (400+100)×3 = **1500** |
| R2 | 150 | 0（ノックアウト） | **0** |
| R3 | 300 | 0（ノックアウト） | **0** |
| 合計 | 850 | | **1500** |
...
> MVP は 1160。P1 は大きく稼いだラウンドを生き延びて 1500 — **P1 の勝ち**です。
...
```python
SURVIVE_BONUS       = 100   # 残りハート 1 つあたりの MVP（0 でボーナス無し）
...
| 生存の価値を上げる | `SURVIVE_BONUS` を上げる（300 ならハート 1 つ = フルーツ 6 個） |
| 生存ボーナスを消す（スコア×HP に戻す） | `SURVIVE_BONUS = 0` |
