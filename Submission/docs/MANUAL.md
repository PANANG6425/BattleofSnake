...
Everything runs from one file: **`snake_battle.py`** — 2447 lines, English comments
throughout, laid out to the baseline kit's five sections. The multi-file version it was
built from is kept in `Code/snake_battle_2P/` and still runs from `main.py`; the two are
the same game.
...
- **Nothing else to install** — the game is `turtle` + the standard library.
  The sprites in `assets/*.gif` and the sounds in `assets/sounds/*.wav` ship with it.
- Optional
...
**Where it looks for art and sound.** On startup the file resolves its own base folder:
the folder it is in if that holds an `assets/` directory, otherwise `snake_battle_2P/`
next to it. So it runs both from the submission folder (art beside it) and from `Code/`
in the repo, with no edit.

Startup prints one sound-status line, e.g. `SOUND: 9 events via winsound`, or an
explanation of why there is no sound and what to do about it.

The font and the window icon are configurable at the top of the file — see 9.4 and 9.5.

---

## 2. What is inside `snake_battle.py`

Top-level blocks run in baseline order: **1 → 2 → 3**, then the screens and the two game
modes, each of which carries its own Sections 2, 3, 4 and 5. Line numbers are exact.

| Lines | Block | Baseline section |
|---:|---|---|
| 1–31 | file docstring and stdlib imports | — |
| 34–89 | window size, title, background, sprite folder, font and icon lists | **1 Screen Setup** |
| 91–95 | `wn = turtle.Screen()` · `title` · `bgcolor` · `setup(800, 600)` · `tracer(0)` | **1 Screen Setup** |
| 97–566 | asset resolution, font and icon pick, sprite loading, scene switching, drawing helpers, compound power icons | **1 Screen Setup** |
| 568–854 | the `Snake` class, the power runtime, fruit | **2 Game Entities** |
| 856–1043 | theme palette, rules, arenas, sound settings, character and power registries | **3 Parameters & Physics** |
| 1045–1317 | `SoundManager` — non-blocking, silent when no audio files exist | — |
| 1319–1764 | Main Menu, Character Select, round and match result screens | 2 + **4 Input** |
| 1766–1969 | `game_1p_scene()` — 1 Player | 3, 2, **4**, **5 Game Loop** |
| 1971–2437 | `game_2p_scene()` — 2 Player Battle | 3, 2, **4**, **5 Game Loop** |
| 2442–2447 | startup: print the sound report, open the menu, `wn.mainloop()` | — |

Sub-headings inside those blocks (`SECTION 1A`…`1F`, `2A`…`2C`, `3A`…`3H`, and
`SECTION 2 / 3 / 4 / 5` inside each game mode) are marked with banner comments in the
code, so a search for `# SECTION` lists the whole structure.

**Landmarks**

| Line | Name | What it is |
|---:|---|---|
| 44 | `WIN_W, WIN_H` | window size |
| 54 | `FONT_CANDIDATES` | font fallback list (see 9.4) |
| 91 | `wn = turtle.Screen()` | the single Screen — nothing may create another |
| 95 | `wn.tracer(0)` | manual redraw; every scene calls `wn.update()` itself |
| 176 | `load_shape()` | GIF loading with a failure cache |
| 239 | `clear_all_turtles()` | the scene-change cleanup (see 10) |
| 283 | `go_to_scene()` | epoch-guarded scene switch |
| 629 | `class Snake` | movement, growth, skill meter, powers |
| 869 | THEME block | every colour in the game |
| 911 / 919 | `ARENA_1P` / `ARENA_2P` | arena rectangles, `(left, right, bottom, top)` |
| 960 | `SOUND_ON` | sound settings block |
| 982 | `CHARACTERS` | character registry (see 4) |
| 1009 | `POWERS` | power registry (see 5) |
| 1118 | `class SoundManager` | pygame → winsound → CLI fallback chain |
| 1387 | `menu_scene()` | Main Menu |
| 1426 | `character_select_scene()` | Character Select, both modes |
| 1654 | `mvp_total()` | the MVP formula (see 7) |
| 1778 | `game_1p_scene()` | 1 Player |
| 1980 | `game_2p_scene()` | 2 Player Battle |

**Four ordering rules the file depends on**

- The Section 1 constants come before the Screen because `wn.title()` and `wn.setup()`
  read them. `TEXT_BRIGHT` is up there too: it is a default argument of
  `ImageButton.__init__`, and Python evaluates defaults when the class is created.
- The Screen (line 91) is created before any `Turtle`. turtle allows one per process.
- Section 3 (line 856) sits after Section 2 because the `Snake` class only *reads*
  those constants inside its methods, at call time. `SoundManager` (line 1045) is the
  opposite — it touches `SOUND_*` while the module loads, so it must come after Section 3.
- Each game mode keeps its own layout constants **inside** its function
  (`ARENA_L`, `WALL_L`, `BAR_W`, `HINT` …). 1P and 2P use the same names with different
  values, so they cannot sit at module level. Nested helpers read them as closures —
  which is also why Section 3 precedes Section 2 *within* a mode.

> The multi-file version and its import graph are documented in
> [snake_battle_2P/CODE_REVIEW.md](../Code/snake_battle_2P/CODE_REVIEW.md).
...
**Adding a character** — append an entry to `CHARACTERS` (line 982) and run
`python make_sprites.py` from `Code/snake_battle_2P/`. Character Select reads the count from the registry itself
...
**Adding a power** — `POWERS` (line 1009) writes each `effect` as EFFECT KINDS the
...
it with `powers_flag()` / `powers_effect()` (line 611) at the one place that
...
|---|---|---|
| `eat` | fruit collected | 1P · 2P |
| `hit` | HP lost (enemy hit or self hit) | 2P |
| `bump` | hit a wall or a box | 2P |
| `power` | a power actually fired | 1P · 2P |
| `win` | the RESULT screen with a winner | 2P |
| `lose` | GAME OVER in 1P | 1P |
| `select` | character / power picked | menu |
| `start` | a match begins | menu · 1P · 2P |
...
The extra folders are configurable at line 965:
...
**For live detail** set `SOUND_DEBUG = True` (line 966); every failed playback
...
```python
SOUND_ON     = True                     # line 960 - start unmuted
SOUND_DIR    = 'sounds'                 # subfolder of assets/
SOUND_VOLUME = 0.6                      # 0.0-1.0 (pygame backend only)
EXTRA_SOUND_DIRS = ['../../sound_effect']
```

`SoundManager` may never raise and never block — a missing file, a broken file or a busy
...
`SELF_HIT_DAMAGE = 0` (line 942) for stun only, no HP loss.
...
Almost every knob is in Section 3, lines 856-1043. The exceptions are listed below it.

| To change | Where |
|---|---|
| window size · title · background · sprite folder · font · icon | lines 44-89 (Section 1A) |
| frame time (`FRAME_MS = 16`, about 60 FPS) | line 46 |
| **every colour in the game** (wall, boxes, text, cards, buttons) | the THEME block, line 869 |
| shared rules: segment spacing, growth, fruit score, skill cap | line 892 onwards |
| 1P arena and speed | lines 911-914 |
| 2P arena, HP, target score, rounds, stun, i-frames, obstacles, fruit | lines 919-955 |
| sound: on/off, folder, volume, extra folders, debug | lines 960-966 |
| characters: colour + sprite palette | `CHARACTERS`, line 982 |
| powers: effect, cost, duration, icon, blurb | `POWERS`, line 1009 |
| per-mode HUD / heart / bar positions | the constants at the top of `game_1p_scene()` (1778) or `game_2p_scene()` (1980) |
| snake spawn points | `SPAWN`, inside `game_2p_scene()` |
...
| MVP to ignore HP (raw points again) | make `mvp_total()` (line 1654) `sum(r[slot] for r in rounds)` |
...
Shared constants are declared once, in Section 3 — change them there and both modes
follow. Only the per-mode layout numbers live inside each mode's function.
...
changes**. `<key>` is `p1` `p2` `p3` `p4` per the `CHARACTERS` list (line 982).
...
- Set `USE_SPRITES = False` (line 49) to force coloured squares everywhere
...
| ✅ | ❌ | **head from your art, body a coloured square from the character registry** ← this one |
...
> character's `main` colour, editable in `CHARACTERS` without touching any image.
...
`FONT_CANDIDATES` (line 54) is a **list**, not a single name, because tkinter
silently substitutes a different font when the one you asked for is missing — so you
never find out what you actually got. `_pick_font()` (line 110) asks Tk which families exist and
...
- **A shape turtle always covers text.** turtle redraws a turtle's cursor with
  ...
  (`filled_rect()`) with clicks hit-tested (`wn.onscreenclick` + `inside_rect()`).
...
- **Shapes rotate by `heading - 90`** because turtle's built-ins are authored nose-up.
  Shapes here are authored in screen coordinates, so display them through
  `shape_turtle()` (line 435), which sets heading 90.
- **Turtles must be destroyed on a scene change.** `hideturtle()` is not enough: delete
  ...
  leaks. `clear_all_turtles()` (line 239) does this.
...
- **tkinter substitutes fonts silently.** Ask `tkinter.font.families()` what actually
  exists before choosing — `_pick_font()` does.

---

## 11. Where the code came from

`snake_battle.py` is a flattened build of the eight runtime modules in
`Code/snake_battle_2P/`, concatenated in dependency order with:

- every local `from … import …` removed, the stdlib imports hoisted and de-duplicated
- the blocks ordered 1 (Screen Setup) → 2 (Entities) → 3 (Parameters) → screens → modes,
  as far as Python's definition-time evaluation allows
- each mode's colliding layout constants moved inside its own scene function
- the asset folder resolved at runtime instead of assumed next to the file

Every comment is the team's own, carried over from those modules unchanged apart from
cross-file references, which now point at functions instead of filenames.
